...
const Select: React.FC<React.SelectHTMLAttributes<HTMLSelectElement>> = (props) => (
  <select
    ...
    className={
      ...
      "dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:focus:ring-indigo-400 " +
      (props.className ?? "")
    }
  />
);
...
const Card: React.FC<CardProps> = ({ children, title, desc }) => (
  <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm h-full flex flex-col dark:bg-gray-900 dark:border-gray-800">
    {title && (
      <div className="mb-3">
...
export default function App() {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-4 p-4 font-sans text-gray-900 dark:text-gray-100 transition-colors duration-200">
      <header className="flex flex-col md:flex-row items-center justify-between pb-4 border-b border-gray-200 dark:border-gray-800">
        <div className="text-center md:text-left mb-4 md:mb-0">
          <h1 className="text-3xl font-bold tracking-tight sm:text-4xl text-gray-900 dark:text-white">AI Voice Studio</h1>
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">Powered by Google Gemini. Generate multi-lingual speech with SSML support.</p>
        </div>
        <button
          onClick={() => setDarkMode(!darkMode)}
          className="p-2 rounded-lg bg-gray-200 hover:bg-gray-300 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-800 dark:text-gray-200 transition-colors"
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <Card title="Settings" desc="Choose a language, voice, and parameters">
          ...
          <Field>
            ...
            <input id="speed-range" type="range" min={0.5} max={2} step={0.01} value={speed} onChange={e => setSpeed(parseFloat(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700" disabled={isSSML}/>
          </Field>
          <Field>
            <Label htmlFor="pitch-range">Cao độ (Pitch): {pitch} semitones</Label>
            <input id="pitch-range" type="range" min={-20} max={20} step={0.1} value={pitch} onChange={e => setPitch(parseFloat(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer dark:bg-gray-700" disabled={isSSML}/>
          </Field>
          <div className="flex items-center gap-2 mt-4">
            <input id="ssml-checkbox" type="checkbox" checked={isSSML} onChange={e => setIsSSML(e.target.checked)} className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-700"/>
            <label htmlFor="ssml-checkbox" className="text-sm font-medium text-gray-700 dark:text-gray-300">Enable SSML</label>
          </div>
           {isSSML && <p className="mt-1 text-xs text-indigo-600 dark:text-indigo-400">Speed and Pitch controls are disabled when SSML is active.</p>}
        </Card>

        <Card title="Content" desc="Enter plain text or SSML">
          <textarea
            className="h-64 w-full resize-none rounded-lg border border-gray-300 p-3 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono dark:bg-gray-800 dark:border-gray-700 dark:text-white"
          ...
          {violations.length > 0 && (
            <p className="mt-2 text-xs text-red-600 font-medium dark:text-red-400">Warning: Banned words detected - {violations.join(", ")}</p>
          )}
        </Card>
        ...
        <Card title="Result & Controls" desc="Listen and download your audio">
          <div className="flex gap-2">
            ...
            <Button onClick={handleDownload} disabled={!audioUrl} className="bg-gray-600 hover:bg-gray-700 disabled:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 dark:disabled:bg-gray-800">Download WAV</Button>
          </div>
          {error && <p className="mt-2 text-sm text-red-600 dark:text-red-400">{error}</p>}
          <div className="mt-4">
            <audio ref={audioRef} src={audioUrl ?? undefined} controls className="w-full" />
          </div>
          <div className="mt-4 p-3 bg-gray-100 rounded-lg text-xs text-gray-600 dark:bg-gray-800 dark:text-gray-300">
            <p className="font-semibold mb-1 text-gray-800 dark:text-gray-200">SSML Tips:</p>
        ...
        <div className="grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-4">
          {SCENARIOS.map((p) => (
            <button
              ...
              className="rounded-lg border border-gray-200 p-3 text-left text-sm transition hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-800"
            >
              <div className="font-medium text-gray-800 dark:text-white">{p.name}</div>
              <div className="text-gray-600 line-clamp-3 dark:text-gray-400">{p.text}</div>
            </button>
          ))}
        </div>
      </Card>
